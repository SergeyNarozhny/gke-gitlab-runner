locals {
  creds_path = "~/.secrets/gke-fx-prod-sa.json"
  tf_bucket = "terraform-states-fx-prod"
  tf_bucket_path = "common/gke-gitlab-runners-asia-1684164323/"
...
resource "google_storage_bucket" "gitlab_runners_cache" {
  name          = "fx-prod-gitlab-runners-cache"
...
data "google_iam_policy" "storage_policy" {
  binding {
    ...
    members = [
      ...
      "projectEditor:fx-prod",
    ]
  }
  binding {
    role = "roles/storage.objectViewer"
    members = [
      "projectViewer:fx-prod",
    ]
  }
}
...
data "google_service_account" "gitlab_runners_nodes_sa" {
  account_id = "109262727890089609723" # gitlab-runners-nodes-sa, hardcoded to simplify otherwise
}
